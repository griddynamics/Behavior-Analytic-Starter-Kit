$(function(){
    ...
    $('body').on('click', 'input.addToCart,input.addToWishlist', function() {
        ...
        var inFansybox = $button.parents('.fancybox-inner').length > 0;

        var itemRequest = BLC.serializeObject($form),
            productOptionDialog = inFansybox && !$button.hasClass('addRecommend'),
            wishlistAdd = $button.hasClass('addToWishlist'),
            recommendationInCart = inFansybox && $button.hasClass('addRecommend');

        if (itemRequest.hasProductOptions == "true" && !productOptionDialog) {
            $.fancybox.open($.extend({ href : '#productOptions' + itemRequest.productId}, fancyProductOptionsOptions));
        } else if (recommendationInCart) {
            BLC.ajax({
                ...
                function(data, extraData) {
                    if (extraData.error) {
                        ...
                        showInCartButton(extraData.productId, 'cart');
